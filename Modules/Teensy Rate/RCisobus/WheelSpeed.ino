...
DMAMEM uint32_t LastPulseWhl;
DMAMEM uint32_t ReadLastWhl;
DMAMEM uint32_t PulseTimeWhl;
DMAMEM float HzWhl = 0;
const uint32_t PulseMinWhl = 250;		// 4000 Hz
const uint32_t PulseMaxWhl = 1000000;	// 1 Hz
const uint8_t PulseSampleSizeWhl = 11;

DMAMEM volatile uint32_t SamplesWhl[MaxSampleSize];
DMAMEM volatile uint16_t PulseCountWhl;
DMAMEM volatile uint8_t SamplesCountWhl;
DMAMEM volatile uint8_t SamplesIndexWhl;
