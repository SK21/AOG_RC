...
FLASHMEM void ReadPGNs(byte data[], uint16_t len)
{
	byte PGNlength;
			...
			if (GoodCRC(data, PGNlength))
			{
				if (ParseModID(data[2]) == MDL.ID)
				{
					...
					if (SensorID < MDL.SensorCount)
					{
						...
						Machine.AutoOn = ((InCommand & 64) == 64);
	...
	case 32700:
		...
		//29    work pin
		//30    pressure pin
		//31    CommMode             0 - UDP only, 1 - CAN Proprietary, 2 - UDP + CAN Proprietary
		//32    CRC
