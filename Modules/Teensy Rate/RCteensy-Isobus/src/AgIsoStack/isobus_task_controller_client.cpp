...
namespace isobus
{
	...
	void TaskControllerClient::update()
	{
		switch (currentState)
		{
			...
			case StateMachineState::Connected:
			{
				...
				else if (!SystemTiming::time_expired_ms(connectedTimestamp_ms, 3000))
				{
					clear_queues();
				}
				else
	...
	void TaskControllerClient::set_state(StateMachineState newState)
	{
		...
		{
			const std::uint32_t timestamp = SystemTiming::get_timestamp_ms();
			const bool enteringConnected = (StateMachineState::Connected == newState);
			...
			else if (enteringConnected)
			{
				connectedTimestamp_ms = timestamp;
				clear_queues();
			}
		}
	}
	...
	{
		const bool enteringConnected = (StateMachineState::Connected == newState) && (StateMachineState::Connected != currentState);
		stateMachineTimestamp_ms = timestamp;
		currentState = newState;
