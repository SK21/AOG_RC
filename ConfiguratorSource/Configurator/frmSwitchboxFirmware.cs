            ...
            try
            {
                ...
                else
                {
                    if (ckSWethernet.Checked && ckSWOldBootloader.Checked)
                    {
                        ...
                        File.WriteAllBytes(HexFile, Configurator.Properties.Resources.SWOB_ino);
                    }
                    else if (ckSWethernet.Checked && !ckSWOldBootloader.Checked)
                    {
                        // ethernet, new bootloader
                        HexFile = Path.GetTempFileName();
                        File.WriteAllBytes(HexFile, Configurator.Properties.Resources.SW_ino);
                    }
                    else if (!ckSWethernet.Checked && !ckSWOldBootloader.Checked)
                    {
                        // no ethernet, new bootloader
                        HexFile = Path.GetTempFileName();
                        File.WriteAllBytes(HexFile, Configurator.Properties.Resources.SWNE_ino);
                    }
                    else if (!ckSWethernet.Checked && ckSWOldBootloader.Checked)
                    {
                        // no ethernet, old bootloader
                        HexFile = Path.GetTempFileName();
                        File.WriteAllBytes(HexFile, Configurator.Properties.Resources.SWOBNE_ino);
                    }
                }
