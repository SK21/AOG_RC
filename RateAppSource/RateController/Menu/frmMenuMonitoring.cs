...
namespace RateController.Menu
{
    public partial class frmMenuMonitoring : Form
    {
        ...
        public frmMenuMonitoring(frmMenu menu)
        {
            InitializeComponent();
            MainMenu = menu;
            this.Tag = false;
        ...
        private void frmMenuMonitoring_FormClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void UpdateDisplay()
        {
            ...
            lbWidthData.Text = Core.Sections.WorkingWidth(!Props.UseMetric).ToString("N1");
            if (!Props.UseMetric)
            {
                lbWidth.Text = Lang.lgWorkingWidthFT;
            }
            ...
            lbWorkRateData.Text = Core.Sections.WorkRatePerHour().ToString("N1");
            if (!Props.UseMetric)
            {
                lbWorkRate.Text = Lang.lgAcresHr;
            }
            ...
            wifiBar.Value = Core.ModulesStatus.WifiStrength(MainMenu.CurrentProduct.ModuleID);

            // update sections
            for (int i = 0; i < 16; i++)
            {
                Sec[i].Enabled = (Core.Sections.Item(i).Enabled);
