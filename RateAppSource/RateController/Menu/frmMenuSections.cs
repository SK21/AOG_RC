...
namespace RateController.Menu
{
    public partial class frmMenuSections : Form
    {
        ...
        public frmMenuSections( frmMenu menu)
        {
            Initializing = true;
            InitializeComponent();
            MainMenu = menu;
        ...
        private void bntOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (UseZones)
                {
                    ...
                    if (SectionCountChanged || SectionsPerZoneChanged) Core.Zones.Build(SectionsPerZone);
                ...
                Core.Sections.CheckSwitchDefinitions();
                SetButtons(false);
                UpdateForm();
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                switch (e.ColumnIndex)
                {
                    case 2:
                        // end section
                        double.TryParse(val, out tempD);
                        using (var form = new FormNumeric(1, Core.Sections.Count, tempD))
                        {
                            var result = form.ShowDialog();
                            if (result == DialogResult.OK)
                            {
                                ...
                                Core.Zones.Update(Zone - 1, (int)form.ReturnValue, SectionsPerZone);
                                LoadDGV2();
                            }
                        }
                        break;
                    ...
                    for (int j = 1; j < 3; j++)
                    {
                        ...
                        switch (j)
                        {
                            case 1:
                                ...
                                if (!Props.UseMetric)
                                {
                                    Core.Sections.Item(i).Width_inches = (float)Convert.ToDouble(val);
                                }
                                else
                                {
                                    Core.Sections.Item(i).Width_cm = (float)Convert.ToDouble(val);
                                }
                                break;

                            case 2:
                                // switch
                                Core.Sections.Item(i).SwitchID = Convert.ToInt32(val) - 1;    // displayed as 1-16, saved as 0-15
                                break;
                        }
                    }
                }
                        ...
                        for (int j = 1; j < 5; j++)
                        {
                            ...
                            switch (j)
                            {
                                case 3:
                                    ...
                                    if (!Props.UseMetric)
                                    {
                                        Core.Zones.Item(i).Width_inches = (float)Convert.ToDouble(val);
                                    }
                                    else
                                    {
                                        Core.Zones.Item(i).Width_cm = (float)Convert.ToDouble(val);
                                    }
                                    break;

                                case 4:
                                    // switch
                                    Core.Zones.Item(i).SwitchID = Convert.ToInt32(val) - 1;    // displayed as 1-16, saved as 0-15
                                    break;
                            }
                        }
                    }
        ...
        {
            int tmp = 0;
            if (int.TryParse(tbSectionCount.Text, out tmp)) Core.Sections.Count = tmp;
        ...
        private void UpdateForm()
        {
            ...
            SecCount = (byte)Core.Sections.Count;
        ...
        private void UpdateTotalWidth()
        {
            if (!Props.UseMetric)
            {
                lbWidth.Text = Lang.lgWidth + ":  " + (Core.Sections.TotalWidth(true) * 12.0).ToString("N0") + " Inches";
                lbFeet.Text = (Core.Sections.TotalWidth(true)).ToString("N1") + "  FT";
                lbFeet.Visible = true;
                lbUnits.Text = "Inches";
            }
            else
            {
                lbWidth.Text = Lang.lgWidth + ":  " + (Core.Sections.TotalWidth(false) * 100).ToString("N0") + " cm";
                lbFeet.Visible = false;
                lbUnits.Text = "cm";
            }
        }
