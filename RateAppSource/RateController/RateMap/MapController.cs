...
namespace RateController.RateMap
{
    ...
    public static class MapController
    {
        ...
        public static ZoneManager ZnOverlays;
        ...
        public static MapState CurrentMapState { get { return cState; } }
        ...
        private static void Core_AppExit(object sender, EventArgs e)
        {
            Close();
        }
        ...
        public static bool SaveMapImage(string FilePath)
        {
            ...
            if (!CurrentIsVisible) legendManager.Hide();
            return Result;
        }
        ...
        {
            if (Core.GPS.Connected())
            {
                PointLatLng Position = new PointLatLng(Core.GPS.Latitude, Core.GPS.Longitude);
                SetTractorPosition(Position);
                if (Core.Products.ProductsAreOn() && (cState == MapState.Tracking || cState == MapState.Preview))
                {
                    cRateCollector.RecordReading(Position.Lat, Position.Lng, Core.Products.ProductAppliedRates());
                    ZnOverlays.UpdateAppliedOverlay(Core.Products.ProductAppliedRates());
                }

                legendManager.UpdateLBV();
            }
            UpdateVariableRates();
        }
        ...
        private static void UpdateVariableRates()
        {
            try
            {
                ...
                if (!CurrentZone.IsDefined && Core.Products != null)
                {
                    ...
                    int count = 0;
                    foreach (double rate in Core.Products.BaseRates())
                    {
