...
namespace RateController.RateMap
{
    ...
    public class ZoneManager
    {
        ...
        public ZoneManager()
        {
            ...
            MapController.legendManager.LegendUpdated += LegendManager_LegendUpdated;
        }

        private void LegendManager_LegendUpdated(object sender, EventArgs e)
        {
            BuildAppliedFromHistory(cAppliedOverlay);
        }
        ...
        public bool BuildAppliedFromHistory(GMapOverlay overlay)
        {
            try
            {
                ...
                Trail.DrawTrail(overlay, out AveRates, true);
                ...
                if (overlay.Polygons.Count > 0)
                {
                    ...
                    for (int i = 0; i < polygonCount; i++)
                    {
                        ...
                        int productCount = Math.Min(ZoneFields.Products.Length, Props.MaxProducts - 2);
                        for (int p = 0; p < productCount; p++)
                        {
                            rates[ZoneFields.Products[p]] = 0.0;
                        }
        ...
        public bool BuildNewAppliedZones(out List<MapZone> NewAppliedZones)
        {
            ...
            try
            {
                ...
                if (RecordedData)
                {
                    ...
                    bool histOk = BuildAppliedFromHistory(AppliedOverlay);
        ...
        public bool CreateZone(string name, double Rt0, double Rt1, double Rt2, double Rt3, double Rt4, Color zoneColor, out int ErrorCode)
        {
            bool Result = false;
            ErrorCode = 0;
            try
            {
                ...
                else
                {
                    ...
                    var rates = new Dictionary<string, double>();
                    double[] rtArray = new double[] { Rt0, Rt1, Rt2, Rt3, Rt4 }; // size must match productCount
                    int productCount = Math.Min(ZoneFields.Products.Length, Props.MaxProducts - 2);
                    for (int p = 0; p < productCount; p++)
                    {
                        rates[ZoneFields.Products[p]] = rtArray[p];
                    }

                    MapZone NewZone = new MapZone(name, polygon, rates, zoneColor, ZoneType.Target);
        ...
        public bool EditZone(string name, double Rt0, double Rt1, double Rt2, double Rt3, double Rt4, Color zoneColor, out int ErrorCode)
        {
            // rates, color
            bool Result = false;
            ErrorCode = 0;
            try
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        double[] rtArray = new double[] { Rt0, Rt1, Rt2, Rt3, Rt4 };
                        int productCount = Math.Min(ZoneFields.Products.Length, Props.MaxProducts - 2);
                        for (int p = 0; p < productCount; p++)
                        {
                            NewRates[ZoneFields.Products[p]] = rtArray[p];
                        }
                ...
                if (productIndex >= 0 && productIndex < ZoneFields.Products.Length)
                {
                    ...
                    if (productIndex < cLookAheadSeconds.Length && cLookAheadSeconds[productIndex] > 0 && speedMetersPerSecond > 0.0)
                    {
                        ...
                        if (lookAheadZone == null)
                        {
                            ...
                            var rates = Core.Products.BaseRates();
            ...
            try
            {
                if (cShowApplied)
                {
                    ...
                    if (!OverlayIsCurrent)
                    {
                        ...
                        if (BuildAppliedFromHistory(cAppliedOverlay))
                        {
                        ...
                        switch (TuningState[ProductID])
                        {
                            ...
                            case 1:
                                ...
                                if ((DateTime.Now - TuningTimeStart[ProductID]).TotalSeconds < 20)
                                {
                                    ...
                                    double diff = Math.Abs(LZR - Core.Products.Item(ProductID).CurrentRate());
